...
class vs2RuleGroup:
    ...
    def create_rule_group(
        ...
        conn.execute(
            text(
                """
                insert into value_sets_new.rule_group
                (uuid, vs_version_uuid, is_template, created_from_template, title, description, order, include)
                values
                (:uuid, :vs_version_uuid, :is_template, :created_from_template, :title, :description, :order, :include)
                """
            ),
            {
                "uuid": uuid,
                "vs_version_uuid": vs_version_uuid,
    ...
    def expand_rule_group(self):
        conn = get_db()

        rules = []

        rule_group_data = conn.execute(
            text(
                """
            select * 
            from value_sets_new.value_set_rule vsr
            join value_sets_new.rule_group rg
            on vsr.rule_goup=rg.uuid
            where rg.uuid=:uuid
            """
            ),
            {"uuid": self.uuid},
